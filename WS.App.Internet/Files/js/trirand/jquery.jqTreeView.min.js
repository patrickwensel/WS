...
"use strict";
	...
	TreeView.prototype.renderNode = function(nodeOptions, parent) {
		...
		if (nodeOptions.nodeTemplateID) {
			templateID = nodeOptions.nodeTemplateID;
		}
		if (templateID) {
			text = this.renderTemplate(templateID, nodeOptions);
		}
				...
				this.executeInContext(this, 
					function(e) {
						if (this.leftMouseButtonPressed) {
							...
							if (nodeOptions.selected) {
								$.each(this.self.find(".ui-state-highlight"), 
									this.executeInContext(this, 
										function(i, node) {														
			...
			if (destNode) {
				if (this.draggedNodes.length > 0) {					
					if (this.options.onNodesDropped) {
						var eventArgs = new Object();
						eventArgs.draggedNodes = this.draggedNodes;
						eventArgs.sourceTreeView = this; // not really!
						eventArgs.destinationNode = destNode;
						eventArgs.destinationTreeView = this;
					...
					var draggedNode = this.draggedNodes[0];
					...
					if (this.options.dragAndDropUrl) {
						...
						for (var i=0; i<this.draggedNodes.length; i++) {
							draggedNodes.push(this.serializeNode(this.draggedNodes[i]));
						}
						
						args.draggedNodes = draggedNodes;
						args.sourceTreeViewID = this.options.id; // not really!
						args.destinationNode = this.serializeNode(destNode);
						args.destinationTreeViewID = this.options.id;			
						...
						$.ajax({
							url: this.options.dragAndDropUrl,
							type: "GET",
	...
	TreeView.prototype.cloneSelectedNodes = function(node, nodeOptions, event) {
			var dragDiv = $("<div />")
										...
										.addClass("ui-jqtreeview");
			var dragUl = $("<ul />");
			dragDiv.append(dragUl);

			if (nodeOptions.selected) {
				$.each(this.self.find(".ui-state-highlight"), function(i, node) {
					var clonedNode = $(node).parents("table:eq(0)").clone();			
	...
	TreeView.prototype.getDestinationNode = function() {		
		for (var i=0; i<jqTreeViewInstances.length; i++) {
			...
			if (treeView.hoveredNode) return treeView.hoveredNode;
		}
		return null;
	};
